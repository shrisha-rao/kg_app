#src/services/vector_db/vertex_ai_matching_engine_OLD.py
import logging
import time
...
class VertexAIMatchingEngineService(VectorDBService):
    ...
    def _get_deployed_index_id(self) -> str:
        ...
        logger_debug.info("inside _get_deployed_index_id function")
        logger_debug.info(
            f"setting vertex ai deployed index to {settings.vertex_ai_deployed_index_id}"
        )
    ...
    async def search(
        ...
        try:
            # Execute the search - pass the query embedding directly as a list of floats
            response = self.index_endpoint.find_neighbors(
                deployed_index_id=self.deployed_index_id,
                queries=[query_embedding
                         ],  # Pass the embedding vector directly
                num_neighbors=top_k)

            # Format results and filter by namespace if provided
            results = []
            for neighbor in response[0]:  # First query results
                # Check if the datapoint belongs to the requested namespace
                datapoint_namespace = None
                if (hasattr(neighbor.datapoint, 'restricts')
                        and neighbor.datapoint.restricts):
                    for restriction in neighbor.datapoint.restricts:
                        datapoint_namespace = restriction.namespace
                        break

                # If namespace is specified, only include results from that namespace
                if not namespace or datapoint_namespace == namespace:
                    results.append({
            ...
            # If we filtered out too many results, we might need to do a second search
            # with more neighbors to get the desired top_k
            if namespace and len(results) < top_k:
                self.logger.warning(
                    f"Namespace filter reduced results from {len(response[0])} to {len(results)}. "
                    f"Consider increasing initial num_neighbors for better results with namespace filtering."
                )
    ...
    async def get_index_stats(self) -> Dict[str, Any]:
        ...
        try:
            if self.index_endpoint.deployed_indexes:
                index_stats = self.index_endpoint.deployed_indexes[
                    0].index_stats
                return {
                    "vectors_count":
                ...
                self.logger.warning("No deployed indexes found")
                return {}
        except Exception as e:
...
#             index_stats = settings.vertex_ai_deployed_index_id.index_stats
#             return {
#                 "vectors_count":
#                 index_stats.vectors_count,
#                 "shards_count":
